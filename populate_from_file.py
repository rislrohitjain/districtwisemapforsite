...
# Coordinate Fallbacks in case geocoding fails
DISTRICT_COORDS = {
...
def geocode_district(district_name):
    ...
    req = urllib.request.Request(url, headers={'User-Agent': 'LRSOSFileGeocodePopulator/1.0'})
    try:
...
def main():
    ...
    if not os.path.exists(filename):
        print(f"Error: Please create a '{filename}' file in the workspace directory and paste your untruncated school list inside it.")
        print("Each line in the file should represent a school name.")
        return
        
    print(f"Reading schools from {filename}...")
    ...
    for line in lines:
        ...
        if clean and clean != "college_name" and not clean.startswith("add all"):
            schools.append(clean)
            
    if not schools:
        print("No valid school names found in schools.txt.")
        return
        
    print(f"Found {len(schools)} schools to process.")
    
    # Map schools to districts and geocode unique ones
    ...
    try:
        ...
        conn.commit()
        print(f"Success! Truncated and populated {cur.rowcount} schools.")
