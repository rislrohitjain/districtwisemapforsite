...
# Aliases/mappings for towns or alternate spellings to districts
MAPPING_ALIASES = {
...
DISTRICT_COORDS = {
    ...
    'Jalore': (25.3500, 72.6300),
    'Jhunjhunu': (28.1300, 75.4000),
    'Jhalawar': (24.6000, 76.1500),
    'Jodhpur': (26.2743, 73.0243),
    'Kota': (25.1800, 75.8300),
    'Nagaur': (27.2000, 73.7300),
    'Pali': (25.7700, 73.3300),
    'Sawaimadhopur': (25.9800, 76.3800),
    'Sikar': (27.6200, 75.1500),
    'Sirohi': (24.8800, 72.8500),
    'Tonk': (26.1600, 75.7800),
    'Udaipur': (24.5800, 73.7100),
    'Dholpur': (26.7000, 77.9000),
    'Dausa': (26.8800, 76.3300),
    'Baran': (25.1000, 76.5000),
    ...
    'Balotra': (25.8300, 72.2300),
    'Beawar': (26.1000, 74.3200),
    'Didwana': (27.4000, 74.5700),
    'Kuchaman': (27.1500, 74.8600),
    'Deeg': (27.4700, 77.3300),
    'Gangapur': (26.4700, 76.7200),
    'Kekri': (25.9700, 75.1500),
    'Salumbar': (24.1300, 74.0400),
    'Shahpura': (25.6300, 74.9300),
    'Khairthal': (27.8000, 76.6300),
    'Kotputli': (27.7000, 76.2000),
    'Sawai Madhopur': (25.9800, 76.3800)
}
...
def geocode_district(district_name):
    ...
    req = urllib.request.Request(url, headers={'User-Agent': 'LRSOSLargeGeocodePopulator/1.0'})
    try:
...
def main():
    # 1. Load temp_schools.txt and extract school names
    with open('d:/projects/python/districtwisemapforsite/temp_schools.txt', 'r', encoding='utf-8') as f:
        text = f.read()
    
    matches = re.findall(r'"([^"\n]+)"', text)
    schools = [m for m in matches if m != "college_name"]
    
    print(f"Parsed {len(schools)} schools from logs.")
    
    # 2. Map schools to districts and find unique ones
    school_district_pairs = []
    unique_districts = set()
    for school in schools:
        dist = get_school_district(school)
        school_district_pairs.append((school, dist))
        unique_districts.add(dist)
        
    print(f"Identified {len(unique_districts)} unique districts: {list(unique_districts)}")
    
    # 3. Geocode unique districts
    district_map = {}
    for dist in unique_districts:
        district_map[dist] = geocode_district(dist)
        time.sleep(1.2) # Delay to respect Nominatim policy
        
    # 4. Prepare records with coordinate jittering
    prepared_records = []
    for school, dist in school_district_pairs:
        base_lat, base_lng = district_map.get(dist, (26.55, 74.90))
        # Add random scatter so schools do not overlay directly
    ...
    try:
        ...
        conn.commit()
        print(f"Successfully truncated and populated {cur.rowcount} schools.")
